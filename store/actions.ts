...
export const publishPost = (post: PostPreview): PublishPostAction => ({ post, type: PUBLISH_POST }),
    ...
    setPosts = (posts: PostPreview[]): SetPostsAction => ({ posts, type: SET_POSTS }),
    accessLocalStorage = (): AccessLocalStorageAction => ({ type: ACCESS_LOCAL_STORAGE });

export type PostAction = PublishPostAction | SavePostDraftAction | SetPostsAction | AccessLocalStorageAction;
